...
export class Automobilista implements OnInit {
  ...
  constructor(
    ...
    private cdr: ChangeDetectorRef // Inietta il ChangeDetectorRef per forzare l'aggiornamento della view
  ) {}
  ...
  ngOnInit(): void {
    ...
    this.sinistriService.obsSinistri.subscribe({
      next: (data: any) => {
        ...
        // Forza Angular ad aggiornare la view immediatamente quando arrivano i dati HTTP,
        // senza aspettare un evento utente (es. click sulla searchbar) che triggeri la change detection
